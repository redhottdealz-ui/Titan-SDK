...
class TitanClient:
    ...
    def job_started(self, name, message=None, metadata=None, metrics=None):
        """Start a scheduled/one-shot Titan job and publish startup telemetry.

        This helper is intended for GitHub Actions, cron jobs, and workers that
        run a bounded task and exit instead of staying online as a daemon.
        It returns a TitanJob object so callers can finish with job.success()
        or job.fail(error), while Mission Control receives an immediate
        status/event/heartbeat update.
        """
        job = self.begin_job(name, metadata=metadata)
        current_state = message or f"{name} started."

        job_metrics = {
            "job_name": name,
            "job_status": "running",
            "job_started_at": job.started_at_iso,
            "job_metadata": metadata or {},
        }
        if metrics:
            job_metrics.update(metrics)

        self.merge_metrics(job_metrics)
        self.status(status="running", current_state=current_state, metrics=job_metrics)
        self.event(f"{name} started", current_state, level="info")
    ...
    def job_progress(self, name, current_state, message=None, metrics=None, level="info"):
        """Publish progress for a scheduled/one-shot job."""
        progress_metrics = {
            "job_name": name,
            "job_status": "running",
        ...
        self.status(status="running", current_state=current_state, metrics=progress_metrics)
        self.event(f"{name} progress", message or current_state, level=level)
        return progress_metrics

    def job_completed(self, name, message=None, metrics=None, elapsed_seconds=None):
        """Publish completion telemetry for a scheduled/one-shot job.

        This method is useful when the caller does not keep the TitanJob object.
        If a TitanJob object is available, job.success(...) is still preferred.
        """
        self.increment("jobs_completed")
        if elapsed_seconds is not None:
            self.record_timer(name, float(elapsed_seconds))
        ...
        if elapsed_seconds is not None:
            completion_metrics["job_elapsed_seconds"] = round(float(elapsed_seconds), 2)
        if metrics:
            completion_metrics.update(metrics)

        self.last_successful_job = {
            "name": name,
            "message": message or "Job completed successfully.",
            "elapsed_seconds": completion_metrics.get("job_elapsed_seconds"),
            "finished_at": completed_at,
            "metadata": completion_metrics,
        }

        self.merge_metrics(completion_metrics)
        self.event(f"{name} completed", message or f"{name} completed successfully.", level="success")
        self.status(status="healthy", current_state="Waiting for next scheduled run", metrics=completion_metrics)
        self.metrics(completion_metrics)
        return completion_metrics

    def job_failed(self, name, error, message=None, metrics=None, elapsed_seconds=None):
        """Publish failure telemetry for a scheduled/one-shot job."""
        ...
        if elapsed_seconds is not None:
            failure_metrics["job_elapsed_seconds"] = round(float(elapsed_seconds), 2)
        if metrics:
        ...
        self.last_failed_job = {
            ...
            "message": message or error_text,
            "error": error_text,
            "elapsed_seconds": failure_metrics.get("job_elapsed_seconds"),
            "finished_at": failed_at,
            "metadata": failure_metrics,
        }

        self.merge_metrics(failure_metrics)
        self.event(f"{name} failed", message or f"{name} failed: {error_text}", level="error")
        self.status(status="error", current_state=f"{name} failed", metrics=failure_metrics)
        self.metrics(failure_metrics)
